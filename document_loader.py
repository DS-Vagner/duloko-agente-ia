"""
Leitura e "chunking" (fragmentação) de documentos fonte (PDF ou CSV).

Cada chunk vira uma unidade de recuperação no índice vetorial. O tamanho do
chunk e a sobreposição são configuráveis via app.config (CHUNK_SIZE / CHUNK_OVERLAP).
"""
import csv
...
def load_csv(path: Path) -> str:
    """Converte cada linha do CSV em uma frase 'coluna: valor', concatenando linhas."""
    rows_text = []
...
def load_documents(data_dir: Path) -> List[Chunk]:
    """
    Lê todos os arquivos .pdf e .csv de data_dir e retorna a lista de chunks
    prontos para serem indexados.
    """
